...
func NewEventStoreDB() (*EventStoreDB, error) {
	...
	dbEventStore, err := sql.Open("postgres", "postgresql://root:password@localhost:5432/eventstoredb?sslmode=disable")
	if err != nil {
		return nil, err
	}

	err = dbEventStore.Ping()
	if err != nil {
		return nil, err
	}
	return &EventStoreDB{
...
func NewOrdersDB() (*OrdersDB, error) {
	...
	dbOrders, err := sql.Open("postgres", "postgresql://root:password@localhost:5432/ordersdb?sslmode=disable")
	if err != nil {
		return nil, err
	}
